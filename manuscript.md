...
**Background:** The Myers-Briggs Type Indicator (MBTI) is a published, licensed instrument. 16Personalities, a free website returning four-letter type codes, states on its own pages that it is not the MBTI and uses a different framework. Whether the peer-reviewed literature keeps that distinction has not been measured.

**Methods:** Cross-sectional attribution audit. Works were retrieved whose title or abstract names the MBTI and whose full text contains a 16Personalities word form — an intersection, and therefore an enriched sample by construction, not a sample of MBTI papers generally. OpenAlex and Europe PMC (2015 onwards, retrieved 19 August 2026) returned 118 records and 99 distinct works, of which 58 were journal articles. Full text was obtained for 61 works and 42 of the 58 journal articles; refusals were recorded rather than circumvented, making the open-access frame a declared one. A protocol written before any work was classified fixed the codes, the reporting for each result pattern, and seven sensitivity arms. Two language models of adjacent tiers from one vendor coded every work independently; the author adjudicated all 31 contested items.

**Results:** Among the 27 works in the main analysis that administered an instrument, **17 (63.0%, 95% confidence interval [CI] 44.2–78.5) administered the vendor's test rather than the MBTI**, 1 (3.7%, 0.7–18.3) a published MBTI form, and 9 (33.3%, 18.6–52.2) an instrument the work does not identify. Across all 61 coded works, 44 (72.1%, 59.8–81.8) carried at least one statement conflating the two, and 2 (3.3%, 0.9–11.2) cited the vendor's own pages as psychometric evidence. Thirty-five works could not be retrieved; every figure is a lower bound. All seven sensitivity arms left the conclusion unchanged.

**Conclusions:** In this enriched open-access sample, papers reporting MBTI results usually administered something else, and most reproduced the conflation in prose.
...
The failure reaches research *tools* specifically, and there the shape is closer to ours. Stang, Jonas and Poole (2018) traced the citation history of a 2010 commentary criticising the Newcastle–Ottawa Scale, which had accumulated 1,250 citations through December 2016. In a random sample of 100 citing papers drawn from the Web of Science, 96 were systematic reviews; none quoted the commentary directly, and 94 of the 96 indirect quotations (98%) portrayed it as *supporting* use of the scale when the commentary argued the opposite. A paper arguing against an instrument was cited as endorsing it, by authors who had evidently not read it. The same move — auditing what a body of citations actually rests on, rather than the claim being made — has precedent in fields far from psychology: Sánchez and Parrott (2017) characterised the studies routinely cited as evidence of adverse effects of genetically modified food and feed, asking not whether the claim was true but what its evidence base consisted of.
...
A search of the literature found this specific question unasked. Queries run on OpenAlex on 19 August 2026 for the MBTI in combination with misattribution, conflation or misidentification returned a single record, a 2026 repository deposit; queries for the vendor's word forms in title or abstract returned 58 works, which use the instrument or, in one case, factor-analyse it, but do not audit how the literature attributes it. The general family — citation accuracy, quotation error, citation integrity — returned 705 works and is plainly an established field. The position this paper claims is therefore not that it identifies a new kind of problem, but that it looks for a known one in a place nobody has looked.
...
**The silence.** Nine works administered something and do not say what. This is the failure that reporting guidance already addresses: analyses of published survey-reporting guidance have found the field to be well supplied with guidelines and the practice to be poorly aligned with them (Bennett et al. 2011). A paper that names no form, publisher, version or URL cannot be replicated with respect to its central measurement, and cannot be audited even by a reader who suspects a substitution. In a corpus of this shape the silence is not neutral: it is the state in which substitution is invisible.
...
**What this study does not show.** It does not show that the vendor's test is invalid, and nothing here bears on that question. It does not show that the MBTI is well supported; that question is settled elsewhere and is not ours, though the long-standing critique of the instrument's psychometrics is part of why the substitution matters (Pittenger 2005). It does not show that independent psychometric evidence for the vendor's test is unavailable: a confirmatory factor analysis of the vendor's scale on 1,067 respondents was published in 2020 (Makwana & Dave 2020), five years before the calibration record that sourced reliability coefficients to the vendor's own product page. What can be said is narrower and better supported — such evidence is scarce and hard to find. That paper has six citations, its venue is not in DOAJ, and although its publisher describes it as Scopus-indexed, the journal's Scopus coverage ran 2019–2020 and was discontinued during 2020 at an earlier issue than the one in which the paper appeared. Discoverability, not soundness, is the point; the paper's merits are not at issue here.
...
3. **n₁ = 27, and the interval is wide.** The main analysis rests on 27 works, and the confidence interval on the headline runs from 44.2% to 78.5%. The point estimate should not be quoted without it. The single (b) work in particular carries an interval from 0.7% to 18.3%, and no claim should rest on the observation that only one work administered a published form.

4. **The coders are two tiers of one model line.** Agreement measures within-lineage stability rather than the convergence of independent judgments, and is not comparable to inter-rater reliability between humans. The proposal asymmetry noted in the Discussion cannot be separated from a shared bias by this design.

5. **The protocol was amended three times after coding began, twice at material cost.** Two of the three rounds discarded and re-coded flags on all 61 works. Every amendment is dated and records what had been seen, no amendment followed the computation of a proportion, and the amendments that could raise a rate carry sensitivity arms — but a protocol amended this often is describing a coding task that was under-specified when it began, and the amendment history is reported as a property of the instrument rather than as a defect that was repaired.

6. **Venue class is metadata, and metadata can be wrong about what a document is.** Four retrieved texts are abstracts rather than articles, at least one of them filed as a journal article in a conference supplement. Records were not moved between classes after being read, because that is precisely the boundary shift the planned reporting forbids; S6 measures the effect instead of assuming it, and moves the headline by 1.5 percentage points.

7. **One planned sensitivity arm did not run as designed.** The protocol undertook to code the venue-less record in full so that S2 would run on real codes; the record's full text was never retrieved, and the arm is therefore empty rather than informative.
...
The author declares no competing interests per ICMJE guidelines. The author has no financial or personal relationship with The Myers-Briggs Company, NERIS Analytics Limited, or any provider of personality assessment.
