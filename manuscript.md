...
**Background:** The Myers-Briggs Type Indicator (MBTI) is a published, licensed instrument. 16Personalities, a free website returning four-letter type codes, states it is not the MBTI. Three database searches found no prior audit of whether the literature keeps that distinction.

**Methods:** Cross-sectional audit of works naming the MBTI in title or abstract whose full text contains a 16Personalities word form — an intersection, enriched by construction. OpenAlex and Europe PMC (2015 onwards, retrieved 19 August 2026) returned 99 works, 58 journal articles; 61 were coded, 42 of these. The analytic corpus was conditioned on programmatic retrievability; refusals were recorded, not circumvented. A protocol fixed the codes, the reporting and seven sensitivity arms before any classification; its change log carries nine entries and every departure is reported. Two language models coded every work; the author adjudicated contested items.

**Results:** Of the 27 main-analysis works administering an instrument, **17 (63.0%, model-based Wilson 95% interval 44.2–78.5) administered a vendor-hosted test with no published MBTI form identifiable**, 1 (3.7%) a published MBTI form and 9 (33.3%, 18.6–52.2) an unidentifiable instrument. These codes record what was administered, not what it was called. An **exploratory, unplanned** cross-tabulation gives 15 of the 17 describing it as the MBTI or as descended from it, contrary to the vendor's 2026 description. Across all 61 coded works, 44 (72.1%, 59.8–81.8) carried at least one pre-defined C1–C3 flag and 2 (3.3%) cited vendor pages as psychometric evidence. Thirty-five works were unretrievable: counts are lower bounds, the bias unknown. Five arms ran; none changed the qualitative conclusion. Two were not estimable.

**Conclusions:** In this enriched sample, works naming the MBTI that administered an instrument usually administered a vendor-hosted test with no MBTI form identifiable; in the exploratory cross-tabulation, most described it as the MBTI or as descended from it. Most coded works carried a pre-defined C flag.
...
The protocol's change log carries **nine dated entries**, eight of them after coding began, each recorded with what had been seen at the time. The last four were made after every count existed, and are described at the end of this section. An earlier version of this passage counted six, folding several same-day entries into one; the log itself is the record and the count here is read from it.
...
**Four entries are dated 25 August 2026, and unlike the five before them every count was known when they were written.** They answer three rounds of external review, each conducted on the typeset manuscript alone — without the protocol, the classification file or the repository.
...
carried nine dated entries since. Saying that the reporting "was fixed in advance"
...
| §6's C2 row, which named Jung among the lineages the vendor disclaims | Corrected to the three lineages the vendor does deny; all 24 flagged works re-read against it | Every count known | None; C2 stays at 24 and any C1–C3 at 44 |
...
| Pre-defined C flag | (a) vendor-hosted, no MBTI form identifiable | (b) published MBTI form | (c) not identifiable |
|---|---|---|---|
| **Works** | 17 | 1 | 9 |
...
Four arms warrant a sentence each: two produced no estimate at all, and two returned the main-analysis figures for reasons that are not the same.
...
5. **The protocol's change log carries nine dated entries, eight after coding began, two of them costly and four made after every count existed.** Two rounds discarded and re-coded flags on all 61 works. Every entry is dated and records what had been seen, and those that could raise a rate carry sensitivity arms — but a protocol amended this often describes a coding task that was under-specified when it began, and the history is reported as a property of the instrument rather than as a defect repaired. One consequence reaches the results: the conflation kappas describe the final round only, after rules were written to resolve the disagreements that had depressed them. **The four entries dated 25 August 2026 break the pattern of the five before them**, which all preceded the first proportion: they were written with every count known, and the first of them added the cross-tabulation reported above. No coding was reopened and no planned figure moved, and the addition is labelled exploratory and carries no model-based interval — but a quantity chosen after the numbers are visible cannot carry the guarantee the rest of the protocol was written to provide, and it is not claimed to.
...
**Figure 2.** Instrument administered, among the 27 works in the main analysis that administered an instrument, with model-based Wilson 95% intervals (see Methods): (a) a vendor-hosted test with no published MBTI form identifiable, (b) a published MBTI form, (c) not identifiable from the work.
